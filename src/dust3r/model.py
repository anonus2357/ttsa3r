...
class LocalMemory(nn.Module):
    ...
    def update_mem(self, mem, feat_k, feat_v, return_attn=False):
        ...
        for blk in self.write_blocks:
            ...
            # print("cross_attn maps", cross_attn.shape)
        return mem
    ...
    def _forward_impl(self, views, ret_state=False):
        ...
        for i in range(len(views)):
            ...
            if i  == 0:
                ...
                state_change = (new_state_feat - self.prev_new_state_feat).norm(dim=-1).squeeze(0)  # [768]
                state_change_normalized = state_change / state_change.mean()
                temporal_mask = torch.sigmoid(state_change_normalized - 1.5)  # [768]

                feat_i_norm = feat_i / feat_i.norm(dim=-1, keepdim=True)
                prev_feat_norm = self.prev_feat_i / self.prev_feat_i.norm(dim=-1, keepdim=True)
                feat_dissimilarity = 1.0 - (feat_i_norm * prev_feat_norm).sum(dim=-1)  # [1, 288]

                cross_att = rearrange(torch.cat(cross_attn_state, dim=0),
                                    'l h nstate nimg -> 1 nstate nimg (l h)')[:, :, 1:, :]
                attention_magnitude = cross_att.mean(dim=-1).abs()  # [1, 768, 288]

                interaction = attention_magnitude * feat_dissimilarity.unsqueeze(1)  # [1, 768, 288]
                spatial_signal = interaction.max(dim=-1)[0].squeeze(0)  # [768]
                spatial_mask = torch.sigmoid(spatial_signal)  # [768]

                final_mask = (temporal_mask * spatial_mask).unsqueeze(0).unsqueeze(-1)  # [1, 768, 1]
                update_mask1 = final_mask
