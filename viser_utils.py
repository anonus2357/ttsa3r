...
class PointCloudViewer:
    ...
    def _connect_client(self, client: viser.ClientHandle):
        ...
        wxyz_input = client.gui.add_text(
            "Set wxyz (qx,qy,qz,qw)",
            "0 0 0 1"
        )

        pos_input = client.gui.add_text(
            "Set position (x y z)",
            "0 0 0"
        )

        apply_pose_btn = client.gui.add_button("Apply Pose")

        @apply_pose_btn.on_click
        def _(_) -> None:
            try:
                wxyz = np.fromstring(wxyz_input.value, sep=" ")
                pos = np.fromstring(pos_input.value, sep=" ")
